...
export default function MessageHistoryPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    ...
    },
    {
      id: '7',
      content:
        'I found a critical bug in the checkout process. The payment gateway is not responding properly when using Safari.',
      sender: otherUser.name,
            ...
            {messages.map((message, index) => (
              <div key={message.id}>
                ...
                <div className={`flex ${message.isOwn ? 'justify-end' : 'justify-start'}`}>
                  <div
                    className={`flex items-start space-x-2 max-w-2xl ${message.isOwn ? 'flex-row-reverse space-x-reverse' : ''}`}>
                    {!message.isOwn && (
